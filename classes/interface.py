...
class Interface(object):
    ...
    def travel_menu_handler(self, current_location, destination):
        if(current_location == "baltimore"):
            if(destination == '0'):
                # baltimore --> baltimore
                messagebox.showinfo("Oops!", "You're already in Baltimore! Please choose a different location!")
            if(destination == '1'):
                # baltimore --> frederick
                self.open_scene("travel1")
    ...
    def create_baltimore(self):
        ...
        self.scenes['travel1']['image_label'].place(relx=0.02, rely=0.02)
        self.scenes['travel1']['continue_label'] = tkinter.Label(self.scenes['travel1']['frame'], bg="#1a1a1a", text="CLICK HERE TO CONTINUE", fg="#fff", font=("Arial", 20))
        self.scenes['travel1']['continue_label'].bind( "<Button>", lambda e:self.open_scene('scene7a2'))
        self.scenes['travel1']['continue_label'].place(relx=.15, rely=.9, relwidth=.7)
        self.scenes['travel1']['frame'].place_forget()
  ...
  # create frederick scene
    def create_frederick(self):
        ...
        if(random_event_chance == 1):
            # if random event chance occurs (only if equal to 1), show "slip and fall" message (-2 fun, -2 rest)
            self.scenes['scene8a4']['continue_label'].bind( "<Button>", lambda e:self.frederick_handler("e1"))
        else:
    ...
    def baltimore_handler(self, num):
        if(num == '0'):
            ...
            if("travel1" not in self.visited_scenes):
                self.stats["fun"] += 5
                self.stats["rest"] -= 2
                self.stats["time"] -= 3
                self.stats["hunger"] -= 4
                self.visited_scenes.append("travel1")
                self.open_scene("travel1")
            else:
            ...
            self.travel_menu("baltimore")
        elif(num == 'e1' and "e1" not in self.random_events_experienced):
            ...
            self.open_scene("scene7")
            self.random_events_experienced.append("e1")        
        self.update_stats()
